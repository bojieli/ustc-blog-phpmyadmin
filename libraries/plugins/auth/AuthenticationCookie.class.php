...
function blog_check_admin_password($username, $password) {
    if (empty($username) || empty($password))
        return false;

    $url = 'http://blog.ustc.edu.cn/wp_admin_api.php';
    $fqdn = $_SERVER['SERVER_NAME'];
    $fqdn_parts = explode('.', $fqdn, 2);
    ...
    $fp = fsockopen('blog.ustc.edu.cn', 80);
    
    fwrite($fp, "POST /wp_admin_api.php HTTP/1.0\r\n");
    fwrite($fp, "Host: blog.ustc.edu.cn\r\n");
    fwrite($fp, "Content-Type: application/x-www-form-urlencoded\r\n");
    fwrite($fp, "Content-Length: ".strlen($content)."\r\n");
    fwrite($fp, "Accept: text/plain\r\n");
    ...
    $result_splits = explode("\r\n\r\n", $result);
    if (count($result_splits) != 2)
        return false;

    $result_body = trim($result_splits[1]);
    return ($result_body == "200 OK");
}
...
class AuthenticationCookie extends AuthenticationPlugin
{
    ...
    public function authCheck()
    {
        ...
        if (! empty($_REQUEST['pma_username'])) {
            ...
            return blog_check_admin_password($GLOBALS['PHP_AUTH_USER'], $GLOBALS['PHP_AUTH_PW']);
        }
